...
const email = 'test_admin@klynn.com.do';
...
async function runTest() {
  ...
  const { data: authData, error: authError } = await supabase.auth.signUp({
    email,
    password,
    options: { data: { nombre: 'Test Admin', tenant_id: tenantId, rol: 'ADMIN' } }
  });
  ...
  const { error: dbError } = await supabase.from('empleados').upsert({
    ...
    rol: 'ADMIN',
    activo: true,
    permisos: null, // Admin gets all permissions implicitly
  ...
  console.log("3. Creando cliente autenticado como el admin...");
