...
function ComprobantesPendientesPage() {
  ...
  const ordersById = useMemo(
    () => new Map(orders.map((order: any) => [order.id, order])),
    [orders],
  );
  ...
  const queuedOrderIds = useMemo(
    () => new Set(outbox.map((item) => item.payload?.id || item.entity_id)),
    [outbox],
  );
  const trackedOrderIds = useMemo(
    () => new Set(pendingRemote.map((doc: any) => doc.order_id).filter(Boolean)),
    [pendingRemote],
  );
  const needsAttention = useMemo(
    () =>
      orders.filter((order: any) => {
        ...
        return (
          (status === "ERROR" || status === "PENDING_OFFLINE_TRANSMISSION") &&
  ...
  async function retryQueueItem(item: SyncOutboxItem) {
    if (!online) {
      toast.warning(
        "Todavía no hay conexión. La pre-factura permanece guardada en este dispositivo.",
      );
      return;
    }
  ...
  async function syncAll() {
    ...
    try {
      for (const item of outbox.filter(
        (entry) =>
          entry.status === "failed" || entry.status === "processing" || entry.status === "blocked",
      )) {
        await offlineDB.retryOutboxItem(item.id);
      }
      const result = await syncManager.processQueue(tenantId);
      await refreshAll();
      toast.info(
        `${result.synced} operación(es) sincronizada(s); ${result.failed} pendiente(s) con error.`,
      );
    } finally {
      setSyncingAll(false);
    }
  ...
  if (!hasFiscalModule) {
    return (
      <Card className="p-8 text-center">
        El módulo fiscal no está habilitado para esta lavandería.
      </Card>
    );
  }
      ...
      >
        <div className="flex flex-wrap gap-2">
          ...
          <Button
            ...
            onClick={() => navigate({ to: "/t/$slug/fiscal", params: { slug: tenant!.slug } })}
          >
            <ArrowLeft className="mr-2 h-4 w-4" /> Centro Fiscal
          </Button>
          <Button
            className="rounded-xl bg-[#1B4B73] hover:bg-[#143a59]"
            onClick={syncAll}
            disabled={!online || syncingAll || outbox.length === 0}
          >
            {syncingAll ? (
            ...
            <PendingRow
              ...
              action={
                <Button
                  ...
                  variant="outline"
                  className="rounded-xl"
                  disabled={!online || workingId === item.id}
            ...
            <PendingRow
              ...
              action={
                <Button
                  ...
                  variant="outline"
                  className="rounded-xl"
                  disabled={!online || workingId === doc.id}
                  onClick={() => consultStatus(doc)}
                >
                  {workingId === doc.id ? (
                    <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                  ) : (
...
function SummaryCard({
  ...
  return (
    <Card className="rounded-2xl p-5 flex items-center gap-4">
      <div className={`h-11 w-11 rounded-xl border flex items-center justify-center ${colors}`}>
    ...
    <div className="p-5 sm:p-6 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
      ...
      <div className="flex items-center justify-between gap-4 lg:justify-end">
        {typeof amount === "number" && (
          <span className="font-black whitespace-nowrap">{formatRD(amount)}</span>
        )}
        {action}
      </div>
    </div>
