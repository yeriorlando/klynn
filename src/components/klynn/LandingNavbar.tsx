...
import { getSession, getCurrentUser } from "@/lib/storage";
...
export function LandingNavbar() {
  ...
  useEffect(() => {
    async function checkSession() {
      ...
      const activeSlug = localStorage.getItem("klynn_active_tenant");

      if (session) {
        if (session.tenant_id === "admin" || session.empleado_id === "admin") {
          setActiveSessionUrl("/admin");
          return;
        }
        if (activeSlug && activeSlug !== "admin") {
          setActiveSessionUrl(`/t/${activeSlug}`);
          return;
        }
      }
      ...
      try {
        ...
        if (u) {
          if (u.tenant.id === "admin") {
            setActiveSessionUrl("/admin");
          } else if (u.tenant.slug) {
            setActiveSessionUrl(`/t/${u.tenant.slug}`);
          }
        }
      } catch {}
    }
      ...
      >
        ...
        >
          ...
          <div className="flex items-center gap-2">
            ...
            {activeSessionUrl ? (
              <Link to={activeSessionUrl as any} className="hidden sm:block">
                <Button
                  className="rounded-full bg-[#1B4B73] hover:bg-[#143957] text-white border-0 shadow-xs gap-2 font-bold text-xs h-9 px-4.5 transition-all hover:scale-[1.02] active:scale-[0.98] cursor-pointer"
                >
                  <LayoutDashboard className="h-4 w-4 text-[#F0B900]" />
              ...
              <>
                ...
                <a
                  href="https://wa.me/18299416546?text=%C2%A1Hola!%20%F0%9F%91%8B%2C%20estoy%20interesado%20en%20adquirir%20Klynn%20o%20en%20obtener%20m%C3%A1s%20informaci%C3%B3n%20al%20respecto."
                  target="_blank"
                  rel="noopener noreferrer"
                  className="hidden sm:block"
                >
                  <Button className="btn btn--yellow btn--nav rounded-full font-bold border-0 gap-1.5 cursor-pointer">
          ...
          <div className="fixed inset-0 z-[60] lg:hidden">
            ...
            >
              ...
              <div className="space-y-2 border-t border-slate-100 bg-white p-4 pt-4 pb-12 shadow-inner">
                {activeSessionUrl ? (
                  <Link to={activeSessionUrl as any} onClick={() => setMobileOpen(false)} className="block">
                    <Button className="h-11 w-full bg-[#1B4B73] hover:bg-[#143957] text-white border-0 font-extrabold rounded-xl shadow-xs text-sm gap-2">
                      <LayoutDashboard className="h-4 w-4 text-[#F0B900]" />
                      <span>Volver al dashboard</span>
